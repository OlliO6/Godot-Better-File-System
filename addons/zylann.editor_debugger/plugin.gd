...
func _enter_tree():
	_dock = load("res://addons/zylann.editor_debugger/dock.tscn").instantiate()
	_dock.connect("node_selected", Callable(self, "_on_EditorDebugger_node_selected"))
	add_control_to_dock(DOCK_SLOT_RIGHT_UL, _dock)
	
	var editor_settings = get_editor_interface().get_editor_settings()
	editor_settings.connect("settings_changed", Callable(self, "_on_EditorSettings_settings_changed"))
...
func _on_EditorSettings_settings_changed():
	...
	var tree: Tree = _dock.get_tree_view()
